...
def gomanga_Url_Check(input_url,current_directory, logger, sortingOrder):
    ...
    gomanga_single_regex = compile('https?://(?P<host>gomanga.co)/reader/read/(?P<comic_single>[\d\w-]+)/en/(?P<volume>\d+)?/(?P<Chapter>\d+)?()|(/page/(?P<PageNumber>\d+)?)')
    gomanga_whole_regex = compile('^https?://(?P<host>gomanga.co)/reader/(?P<series>series)?/(?P<comic>[\d\w-]+)?(\/|.)$')

    lines = input_url.split('\n')
    for line in lines:
        found = search(gomanga_single_regex, line)
        if found:
        ...
        found = search(gomanga_whole_regex, line)
        if found:
            match = found.groupdict()
            if match['comic']:
                url = str(input_url)
